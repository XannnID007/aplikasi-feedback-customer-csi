...
@section('title', 'Tambah Umpan Balik Manual')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Tambah Umpan Balik Manual</h1>
                <p class="text-gray-600 mt-1">Input umpan balik pelanggan secara manual (untuk data offline/survey)</p>
            </div>
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-yellow-700">
                        <p>Fitur ini untuk input data feedback yang dikumpulkan secara offline (misalnya survey kertas).
                            Untuk feedback online, pelanggan sebaiknya menggunakan form di website utama.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <form action="{{ route('admin.umpan-balik.store') }}" method="POST" class="space-y-6" x-data="feedbackForm()">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="komentar_umum" name="komentar_umum" rows="4"
                        ...
                        placeholder="Komentar, saran, atau keluhan tambahan (opsional)">{{ old('komentar_umum', $umpanBalik->komentar_umum) }}</textarea>
                    @error('komentar_umum')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="text-sm text-gray-600 space-y-1">
                        <p><strong>Dibuat:</strong> {{ $umpanBalik->created_at->format('d F Y, H:i') }}</p>
                        <p><strong>Terakhir diubah:</strong> {{ $umpanBalik->updated_at->format('d F Y, H:i') }}</p>
                        <p><strong>Rating saat ini:</strong> {{ number_format($umpanBalik->rating_keseluruhan, 1) }}/5.0
                        </p>
                    </div>
                </div>
    ...
    <script>
        function feedbackForm() {
            return {
                ratings: @json(array_merge(old('rating', []), $umpanBalik->detailUmpanBalik->pluck('nilai_rating', 'pertanyaan_id')->toArray())),
            }
        }
    </script>
@endsection
