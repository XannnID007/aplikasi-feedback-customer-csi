...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Tambah Pertanyaan</h1>
                <p class="text-gray-600 mt-1">Buat pertanyaan baru untuk kategori penilaian</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <form action="{{ route('admin.pertanyaan.store') }}" method="POST" class="space-y-6" x-data="questionForm()">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="teks_pertanyaan" name="teks_pertanyaan" rows="3" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 @error('teks_pertanyaan') border-red-500 @enderror"
                        placeholder="Contoh: Bagaimana rasa makanan yang Anda pesan?">{{ old('teks_pertanyaan') }}</textarea>
                ...
                <div>
                    ...
                    <select id="tipe_pertanyaan" name="tipe_pertanyaan" required x-model="questionType"
                        ...
                        <option value="rating" {{ old('tipe_pertanyaan') == 'rating' ? 'selected' : '' }}>Rating (1-5)
                        </option>
                        <option value="teks" {{ old('tipe_pertanyaan') == 'teks' ? 'selected' : '' }}>Teks</option>
                        <option value="pilihan_ganda" {{ old('tipe_pertanyaan') == 'pilihan_ganda' ? 'selected' : '' }}>
    ...
    <script>
        function questionForm() {
            return {
                questionType: '{{ old('tipe_pertanyaan') }}',
                options: [{
                        text: '{{ old('pilihan_jawaban.0', '') }}'
                    },
                    {
                        text: '{{ old('pilihan_jawaban.1', '') }}'
                    }
                ],
